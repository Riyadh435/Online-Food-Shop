...
public class ReviewShowDashBoard implements Initializable {
    @FXML
    private AnchorPane ReviewForm;
    @FXML
    private AnchorPane onlyUserForm;
    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        if(UserDetails.AccountType.equals("user_information")){
            onlyUserForm.setVisible(true);
        }
        DisplayReviewPost();
    ...
    public ObservableList<ReviewData> GetReviewFromYou() {
        ...
        String sql1 = "SELECT id, reviewer_username, reviewer_fullname, reviewer_image, prod_id, prod_name, prod_comment, prod_image,rv_title,rc FROM prod_review WHERE reviewer_username = ?";
        ObservableList<ReviewData> listData = FXCollections.observableArrayList();
    ...
    private void DisplayYourReviewPost() {
        ...
        for (int q = YourReviewListData.size() - 1; q >= 0; q--) {
            ...
            try {
                FXMLLoader load = new FXMLLoader();
                load.setLocation(getClass().getResource("/com/example/e_shop/UserReview/YourReviewCard.fxml"));
                AnchorPane pane = load.load();
                YourReviewCard rv = load.getController();
